...
import mongoose from 'mongoose'; // Не забудьте імпортувати mongoose тут
import Camper from '../models/Camper.js'; // Імпортуємо модель Camper
...
export const addReview = async (req, res) => {
    ...
    try {
        ...
        if (camper.reviews.length > 0) {
            totalRating = camper.reviews.reduce((sum, review) => sum + review.reviewer_rating, 0);
        }
        const averageRating = camper.reviews.length > 0 ? totalRating / camper.reviews.length : 0;
