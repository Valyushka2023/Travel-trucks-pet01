...
function PageCatalog() {
  ...
  if (
    ...
    campers.length > 0 &&
    campers[0].gallery &&
    campers[0].gallery.length > 0
  ) {
    // console.log(
    //   'Перший елемент галереї першого кемпера:',
    //   campers[0].gallery[0]
    // );
    // console.log('Тип першого елемента галереї:',
    typeof campers[0].gallery[0];
  }
  ...
  const filteredCampers = useMemo(() => {
    ...
    filtered = filtered.filter(camper => {
      for (const filterName in activeEquipmentFilters) {
        if (
          Object.prototype.hasOwnProperty.call(camper, filterName) &&
