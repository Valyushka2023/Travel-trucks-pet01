...
import { useDispatch, useSelector } from 'react-redux';
import { useWindowScrollToTopButton } from '../../hooks/useWindowScrollToTopButton.js';
import { useTranslation } from 'react-i18next'; // Імпортовано
import {
  selectCampers,
  selectIsLoading,
  selectError,
} from '../../redux/campers/selectors.js';
import { getCampers } from '../../redux/campers/operations.js'; // Імпортовано операцію
...
function PageCatalog() {
  const { t, i18n } = useTranslation(); // 👈 ЗМІНА: Додано i18n
  ...
  // 💥 ЗМІНА: Передаємо поточну мову в Redux-операцію
  useEffect(() => {
    dispatch(getCampers(currentLang));
  }, [dispatch, currentLang]); // 👈 currentLang додано до залежностей
  ...
  // 💥 ЗМІНА: Передаємо поточну мову в Redux-операцію при повторному запиті
  const handleRetry = () => {
    dispatch(getCampers(currentLang));
  };
